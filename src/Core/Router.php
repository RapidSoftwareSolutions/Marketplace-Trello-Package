...
class Router
{
    ...
    private function setRoute($block)
    {
        ...
        $this->klein->respond('POST', $routePath, function() use($param, $blockName, $blockCustom, $method){
            ...
            if(count($urlPart[0])>0){
                foreach ($urlPart[0] as $onePart) {
                    ...
                    if (isset($inputPram[$oneUrlPart])&&strlen($inputPram[$oneUrlPart])>0) {
                        ...
                        $vendorUrl = str_replace('{{' . $oneUrlPart . '}}', '', $vendorUrl);
                    }
                }
            }
            ...
            // Remove $apiKeys from params
            if(isset($sendParam['apiKeys'])&&strlen($sendParam['apiKeys'])>0){
                $apiKeys = $sendParam['apiKeys'];
                unset($sendParam['apiKeys']);
            }else{
                $apiKeys = false;
            }
            ...
            // Make request
            $result = $this->httpRequest($vendorUrl, $method, $sendBody, $apiKeys, $accessToken);
            echo json_encode($result);
            exit(200);
        });
    ...
    protected function httpRequest($url, $method, $sendBody, $apiKeys, $accessToken)
    {
